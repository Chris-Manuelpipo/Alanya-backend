...
test('arriver demande de RESTER : le premier point dans le rayon ne suffit pas', () => {
  assert.strictEqual(s.checkArrival(1, { ...DANS }, DEST, REGLES, T0), false);
});

test('après l’hystérésis, l’arrivée est détectée', () => {
  s.checkArrival(1, { ...DANS }, DEST, REGLES, T0);
  const r = s.checkArrival(1, { ...DANS }, DEST, REGLES, T0 + 61_000);
...
test('un relevé trop imprécis est ignoré sans annuler l’attente en cours', () => {
  ...
  s.checkArrival(1, { ...DANS, accuracyM: 400 }, DEST, REGLES, T0 + 30_000);
  const r = s.checkArrival(1, { ...DANS }, DEST, REGLES, T0 + 61_000);
...
test('l’arrivée n’est signalée qu’une fois', () => {
  s.checkArrival(1, { ...DANS }, DEST, REGLES, T0);
  assert.strictEqual(s.checkArrival(1, { ...DANS }, DEST, REGLES, T0 + 61_000), true);
  assert.strictEqual(s.checkArrival(1, { ...DANS }, DEST, REGLES, T0 + 90_000), false);
});

test('sans destination, aucune détection', () => {
  assert.strictEqual(s.checkArrival(1, { ...DANS }, null, REGLES, T0), false);
  assert.strictEqual(
    s.checkArrival(1, { ...DANS }, { lat: null, lng: null }, REGLES, T0), false);
});
