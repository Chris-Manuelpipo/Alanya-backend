...
// lecture locale de l'adapter. Voir src/testUtils/fakeIo.js. callState/
// pendingCalls sont async depuis la phase 2 (repli mémoire ici, aucun Redis
// configuré dans ce test — voir src/config/redisData.js).
...
(async () => {
  ...
  await reset();
  await callState.setInCall(2, { callId: '500', peerId: 1 });
  await callState.setInCall(1, { callId: '500', peerId: 2 });
  await pendingCalls.set(2, { callId: '500' });
  assert.strictEqual(await reclaimStaleBusy(fakeIo([]), 2), true, 'orphelin récupéré');
  assert.strictEqual(await callState.get(2), 'idle', 'cible nettoyée');
  assert.strictEqual(await callState.get(1), 'idle', 'pair nettoyé');
  assert.strictEqual(await pendingCalls.get(2), null, 'pending nettoyé');

  // 2) in_call mais EN LIGNE → conservé (appel en arrière-plan légitime).
  await reset();
  await callState.setInCall(2, { callId: '501', peerId: 1 });
  assert.strictEqual(await reclaimStaleBusy(fakeIo([2]), 2), false, 'en ligne = non purgé');
  assert.strictEqual(await callState.get(2), 'in_call', 'en ligne conservé');

  // 3) in_call offline MAIS grâce de reconnexion armée → conservé.
  await reset();
  await callState.setInCall(2, { callId: '502', peerId: 1 });
  ...
  assert.strictEqual(await callState.get(2), 'in_call', 'grâce conservée');
  ...
  await reset();
  await callState.setInCall(2, { callId: '503', peerId: 1 });
  await callState.setInCall(1, { callId: '999', peerId: 3 });
  assert.strictEqual(await reclaimStaleBusy(fakeIo([]), 2), true, 'cible récupérée');
  assert.strictEqual(await callState.get(2), 'idle', 'cible nettoyée');
  assert.strictEqual(await callState.get(1), 'in_call', 'pair non lié conservé');
